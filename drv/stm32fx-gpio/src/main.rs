...
fn turn_on_all_gpios() {
    ...
    for port in [
        ...
        let pnum = Peripheral::GpioA as u32 + port as u32; // see bits in AHB1ENR
        rcc_driver.enable_clock_raw(pnum).unwrap();
        rcc_driver.leave_reset_raw(pnum).unwrap();
    }
}

fn gpio_reg(port: Port) -> &'static device::gpioh::RegisterBlock {
    // TODO Why do we have a/b/h RegisterBlocks? Eyeballing they're all identical, so cast a and b to h.
    match port {
        Port::A => unsafe { &*(device::GPIOA::ptr().cast()) },
        Port::B => unsafe { &*(device::GPIOB::ptr().cast()) },
